...
> **核心发现一句话** - One sentence capturing the most critical signal across all sources
...
## 🎯 Cloud927 洞察
> **趋势判断**: [100+ words on trajectory and implications]
> **行动建议**:
> - [立即可做]
> - [短期规划]
> - [长期观察]
...
"""
    ...
    def _format_cluster(self, category: str, items: list[dict]) -> str:
        ...
        for i, item in enumerate(items[:5], 1):
            ...
            lines.append(f"   - 来源: {source} | 时间: {time_str} | 分数: {score}")
            lines.append("")
...
## 📊 数据概览
- **聚类数量**: {cluster_count}
- **全球要闻**: {sum(len(v) for v in global_news.values())} 篇
- **AI科技**: {sum(len(v) for v in ai_tech.values())} 篇
- **中国AI**: {sum(len(v) for v in china_tech.values())} 篇
- **中国社会**: {sum(len(v) for v in china_society.values())} 篇
...
"""
        # Add clustered data
        priority_categories = ["AI模型更新", "开源工具/框架", "研究论文/突破", "产品发布/融资"]
...
1. **核心发现** - 一句话总结最重要的信号
2. **全球要闻** - 表格展示，按时间排序
3. **AI科技** - 使用聚类结果，按重要性组织
4. **中国动态** - 分为AI前沿和社会重大两部分
5. **趋势追踪** - 展示实体的演变
6. **洞察** - 100+字趋势判断 + 可行动建议

请使用简体中文，表格用于可扫描性，散文用于洞察分析。
"""
